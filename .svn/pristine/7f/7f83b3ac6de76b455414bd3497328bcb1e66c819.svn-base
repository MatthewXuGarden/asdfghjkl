...
/**
        A Multipart form data parser.  Parses an input stream and
        writes out any files found, making available a hashtable of
        other url parameters.  As of version 1.17 the files can be
        saved to memory, and optionally written to a database, etc.
        ...
        @version 1.19 Moved the MultipartRequest into a package, and
            thus into a java archive for easier dissemination.  Fixed
            a bug, where in netscape if a file was uploaded with an
            unrecognised extension, no Content-Type was specified, so
            the first line of the file was chopped off.  Also modified
            the structure of the parse method to make it easier to
            manage.  I was checking strFileName length and nullness in
            two if blocks, so I moved them together.  This should make
            the parse(...) method easier to understand as
            well. 26/07/2001
        @version 1.18 Fixed some serious bugs.  A new method
            readAndWrite(InputStream in, OutputStream out) which now
            does the generic processing in common for readAndWriteFile
            and readFile.  The differences are that now the two extra
            bytes at the end of a file upload are processed once,
            instead of after each line.  Also if an empty file is
            encountered, an outputstream is opened, but then deleted
            if no data written to it.  The getCharArray() method has
            been removed.  Replaced by the new String(bytes, encoding)
            method using ISO8859_1 encoding to ensure that extended
            characters are supported.  All creation of strings is done
            using this encoding.  The addition of static methods
            setEncoding(String) and getEncoding() to allow the use of
            MultipartRequest with a specific encoding type.  All
            instances of MultipartRequest will utilise the static
            charEncoding variable value, that the setEncoding() method
            can be used to set.  Hopefully this will not introduce any
            latent problems. Started to introduce support for multiple
            file uploads with the same form field name, but not
            completed for v1.18.  26/06/2001
        @version 1.17 A few _very_ minor fixes.  Plus a cool new
            feature added.  The ability to save files into memory.
            <b>Thanks to Mark Latham for the idea and some of the
            code.</b> 11/04/2001
        @version 1.16 Added support for multiple parameter values.
            Also fixed getCharArray(...) method to support parameters
            with non-english ascii values (ascii above 127).  Thanks
            to Stefan Schmidt & Michael Elvers for this.  (No fix yet
            for reported problems with Tomcat 3.2 or a single extra
            byte appended to uploads of certain files).  By 1.17
            hopefully will have a resolution for the second problem.
            14/03/2001
        @version 1.15 A new parameter added, intMaxReadBytes, to allow
            arbitrary length files.  Released under the LGPL (Lesser
            General Public License).  03/02/2001
        @version 1.14 Fix for IE problem with filename being empty.
            This is because IE includes a default Content-Type even
            when no file is uploaded.  16/02/2001
        @version 1.13 If an upload directory is not specified, then
            all file contents are sent into oblivion, but the rest of
            the parsing works as normal.
        @version 1.12 Fix, was allowing zero length files.  Will not
            even create the output file until there is something to
            write.  getFile(String) now returns null, if a zero length
            file was specified.  06/11/2000
        @version 1.11   Fix, in case Content-type is not specified.
        @version 1.1 Removed dependence on Servlets.  Now passes in a
            generic InputStream instead.  "Borrowed" readLine from
            Tomcat 3.1 ServletInputStream class, so we can remove some
            of the dependencies on ServletInputStream.  Fixed bug
            where a empty INPUT TYPE="FILE" value, would cause an
            exception.
...
public class MultipartRequest
{
    ...
    // If this Directory spec remains null, writing of files will
    // be disabled...
    ...
    /** Store a read from the input stream here.  Global so we do not
    keep creating new arrays each read.  */
    ...
    /** This method should be called on the MultipartRequest itself,
    not on any instances of MultipartRequest, because this sets up the
    encoding for all instances of multipartrequest.  You can set the
    encoding to null, in which case the default encoding will be
    applied.  The default encoding if this method is not called has
    been set to ISO-8859-1, which seems to offer the best hope of
    support for international characters, such as german "Umlaut"
    characters.

    <b>Warning:</b> In multithreaded environments it is the
    responsibility of the implementer to make sure that this method is
    not called while another instance is be constructed.  When an
    instance of MultipartRequest is constructed, it parses the input
    data, and uses the result of getEncoding() to convert between
    bytes and strings.  If setEncoding() is called by another thread,
    then subsequent string conversions in the private parse() method
    will utilise this new encoding, which will cause serious problems.  */
     ...
     * Constructor.
     *
     * @param strContentTypeText The &quot;Content-Type&quot; HTTP header
     *     value.
     * @param intContentLength The &quot;Content-Length&quot; HTTP header
     *     value.
     * @param in The InputStream to read and parse.
     * @param strSaveDirectory The temporary directory to save the
     *     file from where they can then be moved to wherever by the
     *     calling process.  <b>If you specify <u>null</u> for this
     *     parameter, then any files uploaded will be silently
     *     ignored.</b>
     *
     * @exception IllegalArgumentException If the strContentTypeText
     *     does not contain a Content-Type of "multipart/form-data" or
     *     the boundary is not found.
     * @exception IOException If the intContentLength is higher than
    ...
    {
        this(null, strContentTypeText, intContentLength, in,
             strSaveDirectory, MAX_READ_BYTES);
    }
     ...
     *     parameter, then any files uploaded will be silently
     *     ignored.</B>
     * @param intMaxReadBytes Overrides the MAX_BYTES_READ value, to allow
    ...
    {
        this(null, strContentTypeText, intContentLength, in,
             strSaveDirectory, intMaxReadBytes);
    }
     ...
     * @param strContentTypeText The &quot;Content-Type&quot; HTTP
     *     header value.
     * @param intContentLength The &quot;Content-Length&quot; HTTP
     *     header value.
     * @param in The InputStream to read and parse.
     * @param strSaveDirectory The temporary directory to save the
     *     file from where they can then be moved to wherever by the
     ...
     *
     * @see #MAX_READ_BYTES
     * @deprecated Replaced by MultipartRequest(PrintWriter, String,
     *     int, InputStream, int) You can specify
    ...
    public MultipartRequest(PrintWriter debug,
        ...
        throws IllegalArgumentException, IOException
    {
        this(debug, strContentTypeText, intContentLength, in,
             strSaveDirectory, MAX_READ_BYTES);
    }
        ...
        throws IllegalArgumentException, IOException
    {
        ...
        // Now initialise the object, which will actually call the
        // parse method to parse multipart stream.
     ...
     *     parameter, then any files uploaded will be silently
     *     ignored.</B>
     * @param intMaxReadBytes Overrides the MAX_BYTES_READ value, to
     *     allow arbitrarily long files.
     *
     * @exception IllegalArgumentException If the strContentTypeText
     *     does not contain a Content-Type of "multipart/form-data" or
     *     the boundary is not found.
    ...
    public MultipartRequest(PrintWriter debug,
        ...
        throws IllegalArgumentException, IOException
    {
        // IF strSaveDirectory == NULL, then we should ignore any
        // files uploaded.
     ...
     *
     * @exception IllegalArgumentException If the strContentTypeText
     *     does not contain a Content-Type of "multipart/form-data" or the
     ...
     *
     * If more than one value for a particular Parameter, will return
     * the first.  If an error occurs will return null.  */
     ...
     * although it may actually be empty if not value was encountered
     * for strName or it is an invalid parameter name.  */
     ...
     *  NAMES as encountered during the upload.  */
    ...
    /** If files were uploaded into memory, this method will retrieve
     * the contents of the file as a InputStream.
     *
     * @return the contents of the file as a InputStream, or null if
     * not file uploaded, or file uploaded to file system directory.
     *
     * @see #getFileParameter(java.lang.String, int) */
    ...
    /** Returns a File reference to the uploaded file. This reference
     * is to the files uploaded location, and allows you to
     * read/move/delete the file.
     *
     * This method is only of use, if files were uploaded to the file
     * system.  Will return null if uploaded to memory, in which case
     * you should use getFileContents(strName) instead.
     *
     * @return Returns a null file reference if a call to
     * getFileSize(strName) returns zero or files were uploaded to
     * memory.
     *
    ...
    public File getFile(String strName)
    {
        ...
        // Fix: If fileOutPutDirectory is null, then we are ignoring
        // any file contents, so we must return null.
     ...
     *
     * @param strName is the form field name, used to upload the file.
     *     This identifies the formfield location in the storage facility.
     ...
     *     The getFileSystemName(), getFileSize(), getContentType(),
     *     getContents() methods all use this method passing in a
     *     different type argument.
     *
     * <p><b>Note: </b>This class has been changed to provide for
     * future functionality where you will be able to access all files
     * uploaded, even if they are uploaded using the same form field
     * name.  At this point however, only the first file uploaded via
     * a form field name is accessible.</p>
     *
    ...
    private void parse(InputStream in) throws IOException
    {
        ...
        // First run through, check that the first line is a boundary,
        // otherwise throw a exception as format incorrect.
        ...
        if (strLine==null || strLine.indexOf(this.strBoundary)==-1)
            throw new IOException
                ("Invalid Form Data, no boundary encountered.");

        // At the top of loop, we assume that the Content-Disposition
        // line is next, otherwise we are at the end.
        while (true) {
            ...
            if (read<=0)
                ...
                // and filename from strLine in one go...
                strName = trimQuotes(getValue("name", strLine));

                // If this is not null, it indicates that we are
                // processing a filename.  Now if not null, strip it
                // of any directory information.
                ...
                {
                    // Fix: did not check whether filename was empty
                    // string indicating a FILE was not passed.
                    if (strFilename.length()==0) {
                        ...
                        // FIX 1.14 IE Problem still: Check for
                        // content-type and extra line, even though no
                        // file specified.
                        if (strLine!=null &&
                        ...
                        addFileParameter
                            (strName, new Object[] {null, null, null, null});
                    ...
                    {           // specified, it could still be spurious.
                        ...
                        // Fix 1.11: If not null AND strLine.length()
                        // is long enough.  Modified in 1.19, as we
                        // should be checking if it is actually a
                        // Content-Type.
                            ...
                            strLine.startsWith("Content-Type:"))//Changed 1.19
                        {
                            strContentType = strLine.substring
                                ("Content-Type:".length()).trim();// Chgd 1.13

                            // Skip blank line, but only if a
                        ...
                        // Are we loading files into memory instead of
                        // the filesystem?
                        if (loadIntoMemory) {
    ...
    /** So we can put the logic for supporting multiple parameters
     * with the same form field name in the one location.  */
    private void addParameter(String strName, String value)
    {
        ...
        if (objParms instanceof Vector)
            ...
            // There is only one entry, so we create a vector!
            {
    ...
    /** So we can put the logic for supporting multiple files with the
     * same form field name in the one location.
     *
     * Assumes that this method will never be called with a null
     * fileObj or strFilename.  */
    ...
    /** Read parameters, assume already passed Content-Disposition and
     * blank line.
     *
    ...
    private String readParameter(InputStream in) throws IOException
    {
        ...
        while(true) {
            ...
            // Change v1.18: Only instantiate string once for
            // performance reasons.
        ...
        while(true) {
            ...
            if (read<blockOfBytes.length &&
                    ...
                    (this.strBoundary)!=-1)
                {
                    ...
                    // The secondLineOfBytes will NEVER BE NON-NULL if
                    // out==null, so there is no need to included this
                    // in the test
                    if(sizeOfSecondArray!=0) {
                                // Only used once, so declare here.
                        int actualLength = getLengthMinusEnding
                            (secondLineOfBytes, sizeOfSecondArray);
                ...
                {
                    // Write out previous line.  The sizeOfSecondArray
                    // will NEVER BE ZERO if out==null, so there is no
                    // need to included this in the test
                    if(sizeOfSecondArray!=0)
                        {
                    ...
                    // out will always be null, so there is no need to
                    // reset sizeOfSecondArray to zero each time.
                    if(out!=null)
                        {
                                //Copy the read bytes into the array.
                            System.arraycopy(blockOfBytes,0,
                                             secondLineOfBytes,0,read);
                                // That is how many bytes to read from
                                // the secondLineOfBytes
     ...
     *
     * Assumes that the Content-Disposition/Content-Type and blank
     * line have already been processed.  So we read until we hit a
     * boundary, then close file and return.
     *
        ...
        throws IOException
    {
        ...
        // Do not bother opening a OutputStream, if we cannot even
        // write the file.
        ...
        if (count>0)
            ...
            {
                out.close();
                // Delete file as empty.  We should be able to delete
                // it, if we can open it!
                outFile.delete();
            }
        return count;
    }

    /** If the fileOutPutDirectory wasn't specified, just read the
     * file to memory.
     *
     *  @param strName - Url parameter this file was loaded under.
     *  @return contents of file, from which you can garner the size
     *      as well.  */
    private byte[] readFile(InputStream in) throws IOException
    {
        ...
        if (count>0)
            {
                // Return contents of file to parse method for
                // inclusion in htFiles object.
     ...
     *
     * @param endOfArray This is because in many cases the byteLine
     *     will have garbage data at the end, so we act as though the
     *     actual end of the array is this parameter.  If you want to
     *     process the complete byteLine, specify byteLine.length as
     *     the endOfArray parameter.  */
                                                  ...
                                                  int endOfArray)
    {
        ...
        if (endOfArray>=2 &&
            byteLine[endOfArray-2] == '\r' && byteLine[endOfArray-1] == '\n')
            return endOfArray-2;
    ...
    /** Reads at most READ_BLOCK blocks of data, or a single line
     * whichever is smaller.
     *
     * Returns -1, if nothing to read, or we have reached the
     * specified content-length. Assumes that bytToBeRead.length
     * indicates the block size to read.
     *
     * @return -1 if stream has ended, before a newline encountered
     *     (should never happen) OR we have read past the
     *     Content-Length specified.  (Should also not happen).
     *     Otherwise return the number of characters read.  You can
     *     test whether the number returned is less than
     *     bytesToBeRead.length, which indicates that we have read the
     *     last line of a file or parameter or a border line, or some
        ...
        throws IOException
    {
        ...
        // End of content, but some servers (apparently) may not realise
        // this and end the InputStream, so we cover ourselves this way.
        ...
        // Only if we get actually read something, otherwise something
        // weird has happened, such as the end of stream.
        if (result > 0)
            intTotalRead += result;
    ...
    /**
     * <I>Tomcat's ServletInputStream.readLine(byte[],int,int)
     *    Slightly Modified to utilise in.read()</I>
     * <BR>
     ...
     * array as well.
     *
     * <p>This method <u><b>does not</b></u> returns -1 if it reaches
     *     the end of the input stream before reading the maximum
     *     number of bytes, it returns -1, if no bytes read.
     *
     * @param b an array of bytes into which data is read
     *
     * @param off an integer specifying the character at which this
     *    method begins reading
     *
     ...
     * number of bytes, so we need to check this.  This is the method
    ...
    public String getHtmlTable()
    {
        ...
        sbReturn.append("\n<table border=3><tr><td><b>Name</b></td>"+
                        "<td><b>Value</b></td></tr>");
        ...
        sbReturn.append("\n<table border=2><tr><td><b>Name</b></td>"+
                        "<td><b>Filename</b></td><td><b>Path</b></td>"+
                        "<td><b>Content Type</b></td><td><b>Size</b></td>"+
                        "</tr>");
        for (Enumeration e = getFileParameterNames() ; e.hasMoreElements() ;)
            {
                ...
                sbReturn.append("\n<tr>" +
                                "<td>" + strName + "</td>" +
                                "<td>" + (getFileSystemName(strName)!=null?
                                          getFileSystemName(strName):"") +
                                "</td>");

                if (loadIntoMemory)
                    sbReturn.append("<td>" + (getFileSize(strName)>0?
                                              "<i>in memory</i>":"") +
                                    "</td>");
                else
                    sbReturn.append("<td>" +
                                    (getFile(strName)!=null?
                                     getFile(strName).getAbsolutePath():"") +
                                    "</td>");

                sbReturn.append("<td>" + (getContentType(strName)!=null?
                                          getContentType(strName):"") +
                                "</td>" +
                                "<td>" + (getFileSize(strName)!=-1?
                                          getFileSize(strName)+"":"") +
                                "</td>" +
                                "</tr>");
            }
