...
public class BigEndian implements IEndian
{
    ...
    public char[] concat(byte[] src, int from, char[] dest, int to, int amount)
    {
        ...
        {
            dest[toTmp++] = (char) (((src[fromTmp + 0] & 0xff) << 8) |
                ((src[fromTmp + 1] & 0xff) << 0));
            fromTmp += 2;
        }
    ...
    public short[] concat(byte[] src, int from, short[] dest, int to, int amount)
    {
        ...
        for (int i = 0; i < amount; i += 2)
        {
            dest[toTmp++] = (short) (((src[fromTmp + 0] & 0xff) << 8) |
                ((src[fromTmp + 1] & 0xff) << 0));
            fromTmp += 2;
        }
    ...
    public int[] concat(byte[] src, int from, int[] dest, int to, int amount)
    {
        ...
        {
            dest[toTmp++] = (int) (((src[fromTmp + 0] & 0xff) << 24) |
                ((src[fromTmp + 1] & 0xff) << 16) | ((src[fromTmp + 2] & 0xff) << 8) |
                ((src[fromTmp + 3] & 0xff) << 0));
            fromTmp += 4;
        }
    ...
    public long[] concat(byte[] src, int from, long[] dest, int to, int amount)
    {
        ...
        {
            dest[toTmp++] = (long) (((src[fromTmp + 0] & 0xffL) << 56) |
                ((src[fromTmp + 1] & 0xffL) << 48) |
                ((src[fromTmp + 2] & 0xffL) << 40) |
                ((src[fromTmp + 3] & 0xffL) << 32) |
                ((src[fromTmp + 4] & 0xffL) << 24) |
                ((src[fromTmp + 5] & 0xffL) << 16) |
                ((src[fromTmp + 6] & 0xffL) << 8) |
                ((src[fromTmp + 7] & 0xffL) << 0));
            fromTmp += 8;
        }
    ...
    public int[] concat(char[] src, int from, int[] dest, int to, int amount)
    {
        ...
        for (int i = 0; i < amount; i += 2)
        {
            dest[toTmp++] = (int) (((src[fromTmp + 0] & 0xffff) << 16) |
                ((src[fromTmp + 1] & 0xffff) << 0));
            fromTmp += 2;
        }
    ...
    public long[] concat(char[] src, int from, long[] dest, int to, int amount)
    {
        ...
        for (int i = 0; i < amount; i += 4)
        {
            dest[toTmp++] = (long) (((src[fromTmp + 0] & 0xffffL) << 48) |
                ((src[fromTmp + 1] & 0xffffL) << 32) |
                ((src[fromTmp + 2] & 0xffffL) << 16) |
                ((src[fromTmp + 3] & 0xffffL) << 0));
            fromTmp += 4;
        }
    ...
    public long[] concat(int[] src, int from, long[] dest, int to, int amount)
    {
        ...
        for (int i = 0; i < amount; i += 2)
        {
            dest[toTmp++] = (long) (((src[fromTmp + 0] & 0xffffffffL) << 32) |
                ((src[fromTmp + 1] & 0xffffffffL) << 0));
            fromTmp += 2;
        }
