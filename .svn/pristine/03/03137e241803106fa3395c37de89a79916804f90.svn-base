...
bean.loadSafetyDevices1();

String safety_pressure_pos						= "";
String safety_pressure_series					= "";
String safety_pressure_action					= "";
String safety_pressure_ped_category				= "";
...
Properties[] aRecords							= bean.aSafetyDevices1;  
if( cmd.equals("mod") && aRecords != null && aRecords.length > 0 ) {
	...
	for(int i = 0; i < aRecords.length; i++) {
		if( aRecords[i].getProperty("idrecord").equals(idrecord) ) {
			...
			safety_pressure_pos					= prop.getProperty("safety_pressure_pos");
			safety_pressure_series				= prop.getProperty("safety_pressure_series");
			safety_pressure_action				= prop.getProperty("safety_pressure_action");
			safety_pressure_ped_category		= prop.getProperty("safety_pressure_ped_category");
			...
			readonly							= bean.isSafetyDevices1ReadOnly(Integer.parseInt(prop.getProperty("idrecord"))); 
			break;
		}
	}	
}
...
<table>
	...
	<tr>
		...
		<td width="50%"><input size="64" maxlength="64" name="safety_pressure_pos" value="<%=safety_pressure_pos%>" class="<%=classInput%>" <%=strReadOnly%>></td>
	</tr>
	<tr>
		<td><%=lang.getString("sitebooklet", "type_brand_mod")%></td>
		<td><input size="64" maxlength="128" name="safety_pressure_series" value="<%=safety_pressure_series%>" class="<%=classInput%>" <%=strReadOnly%>></td>
	</tr>
	<tr>
		<td><%=lang.getString("sitebooklet", "pressure_action")%></td>
		<td><input size="64" maxlength="32" name="safety_pressure_action" value="<%=safety_pressure_action%>" class="<%=classInput%>" <%=strReadOnly%>></td>
	</tr>
	<tr>
		<td><%=lang.getString("sitebooklet", "ped_category")%></td>
		<td><input size="64" maxlength="64" name="safety_pressure_ped_category" value="<%=safety_pressure_ped_category%>" class="<%=classInput%>" <%=strReadOnly%>></td>
	</tr>
