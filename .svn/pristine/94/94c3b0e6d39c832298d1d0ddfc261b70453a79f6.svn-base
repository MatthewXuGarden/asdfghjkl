...
public class BSearchAl extends BoMaster
{
    private static final int REFRESH_TIME = -1;
    private static final String EXPORT_PDF_CMD = "exportpdf";

    public BSearchAl(String l)
    {
    ...
    protected Map initializeRefresh()
    {
        ...
        RefreshBean[] rb = 
            {
                new RefreshBean("AlarmFound", IRefresh.R_ALARMSSEARCH, 1, "")
            };
...
//        p.put("tab1name","enableAction(1);enableAction(2);resizeTableAlarmFound();check_limit();");
        p.put("tab1name","enableAction(1);enableAction(2);resizeTableAlarmFound();");
	...
	public String executeDataAction(UserSession us, String tabName,
		...
		AlarmEventList alevList = new SearchAlarmEvent().find(us, -1, true, false,false);
		...
		try {
			...
			if(fileN == null || fileN.equals("")  || fileN.equals("undefined"))
				fileN = "Alarms_" + DateUtils.date2String(new Date(), "yyyyMMddhhmmss");


			exporter.startExport(fileN, langcode);

			exporter.setSiteName(us.getSiteName());
			exporter.setTitle(lan.getString("alrevnsearch", "tab1name"));
			...
			response += "<file><![CDATA[ERROR]]></file></response>";
		}
