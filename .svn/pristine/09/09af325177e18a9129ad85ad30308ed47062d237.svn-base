...
import com.carel.supervisor.director.vscheduler.SchedulerHook;
...
public class BOptStartStop extends BoMaster {
	private static final long serialVersionUID = 3066896753879494510L;
	private static final int REFRESH_TIME = -1;
	
	
	public BOptStartStop(String l)
    {
	...
	protected Properties initializeEventOnLoad()
	{
		...
		p.put("tab4name", "OptimumSchedulerInit();");
		return p;
	}
	...
	{
		Properties p = new Properties();
		p.put("tab1name", "opt_startstop.js;keyboard.js;");
		p.put("tab2name", "opt_startstop.js;keyboard.js;");
		p.put("tab3name", "opt_startstop.js;keyboard.js;");
		p.put("tab4name", "vs.js;opt_startstop.js;keyboard.js;");
		return p;
	}
    ...
    public void executePostAction(UserSession us, String tabName, Properties prop) throws Exception
    {
		if( tabName.equalsIgnoreCase("tab1name") ) {
			...
			if( "start".equals(cmd) )
				OptimumManager.getInstance().getStartStop(idAlgorithm).setEnabled(true);
			else if( "stop".equals(cmd) )
				OptimumManager.getInstance().getStartStop(idAlgorithm).setEnabled(false);
			else if( "refresh".equals(cmd) )
				us.getCurrentUserTransaction().setAttribute("idalg", idAlgorithm);
			else if( "reset".equals(cmd) )
				OptimumManager.getInstance().getStartStop(idAlgorithm).resetCounter();
			else
	...
	public void executeTab2Post(UserSession us, Properties prop) throws Exception
	{
		...
		// retrieve data sets
		StartStopLog.retriveDataSets(idAlgorithm, calBegin, calEnd, cmd, us.getLanguage(), datasetA, datasetB);
	}
	...
	public void executeTab3Post(UserSession us, Properties prop) throws Exception
	{
		...
		if( "save".equals(cmd) ) {
			...
			if( idAlgorithm == 0 )
				OptimumManager.getInstance().addStartStop(bean);
			...
			StartStopBean bean = OptimumManager.getInstance().getStartStop(idAlgorithm);
			bean.delete();
			OptimumManager.getInstance().delStartStop(bean);
    ...
    public String executeTab1Data(UserSession us, Properties prop) throws Exception
	{
    	...
    	/*
    	StartStopBean bean = OptimumManager.getInstance().getStartStop();    	
		...
		response.append("<TimeOff value=\"" + (bean.isEnabled() ? strComputedTimeOff : "***") + "\"/>");
		response.append("</response>");
		*/
	...
	public String executeTab3Data(UserSession us, Properties prop) throws Exception
	{
		...
		switch( action.intValue() ){
			case 0: // LOAD_DEVICE_VAR
				boolean bDigitalVarOnly = ((String)prop.get("bDigitalVarOnly")).equalsIgnoreCase("true");
				...
				for (int i=0;i<vars.length;i++)	{
					...
					if( !bDigitalVarOnly || aux.getType() == 1 ) {
