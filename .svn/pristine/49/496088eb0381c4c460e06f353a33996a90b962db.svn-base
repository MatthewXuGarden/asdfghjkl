...
public class ReorderFrequency
{
    ...
    public void startReorderHistorical() throws Exception
    {
        ...
        for (int i = 0; i < numVariable; i++)
        {
            ...
            newKeyMax =ReorderInformation.calculateNewKeyMax((Integer) objects[i][ReorderInformation.INDEX_ID_VARIABLE],
                    (Integer) objects[i][ReorderInformation.INDEX_HISTORICAL_PERIOD],
                    (Integer) objects[i][ReorderInformation.INDEX_SAMPLING_PERIOD]);
        ...
        /*
        // BIOLO:update productinfo when reorder is finished
       
        updateStatement = connection.prepareStatement("update productinfo set value='TRUE' where key='hsreordered'");
