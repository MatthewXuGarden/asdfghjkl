...
function R_EvnSearch()
{
	if (verifyDate())
	{
		var id_site = document.getElementById("ESid_site").value;
		...
		top.frames['manager'].loadTrx('nop&folder=searchev&bo=BSearchEv&type=click&desc='
									+desc+'&dataselect='+ESgetOptionDate()+'&ESid_site='+id_site+'&datefrom='
									+ESgetDateFrom()+'&dateto='+ESgetDateTo()+'&EScategory='+category+'&remote=yes');
	}
}