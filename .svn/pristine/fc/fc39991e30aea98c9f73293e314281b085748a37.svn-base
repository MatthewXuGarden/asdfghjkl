...
<%UserSession sessionUser = ServletHelper.retrieveSession(request
			...
			String user = lan.getString("evnsearch", "user");
			...
			String ESuser = sessionUser.getProperty("userevent");
			if (ESuser == null)
				ESuser = "";
...
<input type='hidden' id='ESdescriptionPage' value="<%=descriptionPage%>"/>
		...
		<TABLE>
			<TR>
				...
				<TABLE class="standardTxt">
					<TR>
						<TD width="30px"></TD>
						<TD width="60px"><input id="fromto"
							<%=(dataselect.equals("fromto"))?"checked":""%> type="radio"
							name="period" value="from" onclick="<%=(OnScreenKey?"cal1.viewOnly= false;cal2.viewOnly= false;":"enableCal()")%>"><%=from%></TD>
						<TD align="center" valign="top"><input type="hidden" name="tester" id="tester"
							value="" />
						...
						<select class="standardTxt" disabled name="tester_month"
							id="tester_month">
						...
						<input class="standardTxt" disabled type="text"
							name="tester_day" id="tester_day" value="" size="2" maxLength="2" onblur="onlyNumberOnBlur(this);" onkeydown='checkOnlyNumber(this,event);'/>
	...
	<TR>
		...
		<TABLE>
			<TR>
				...
				<TD><input class="<%=(OnScreenKey?"keyboardInput":"standardTxt")%>" type="text" id="userevent"
					value="<%=ESuser%>" size="30" onkeydown='checkBadChar(this,event)' maxlength="30"/></TD>
				<TD width="100px"></TD>
