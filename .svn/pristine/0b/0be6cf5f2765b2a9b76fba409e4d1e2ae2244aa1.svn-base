...
<img alt="Graph" 
	 align="middle" 
	 src="/PlantVisorPRO/SRVLCharts;jsessionid=<%=request.getRequestedSessionId() %>?dtf=<%=request.getParameter("dtf")%>&dtt=<%=request.getParameter("dtt")%>&charttype=<%=request.getParameter("charttype")%>&width=<%=request.getParameter("width")%>&height=<%=request.getParameter("height")%>&imgid=<%=request.getParameter("imgid")%>" />
</body>
</html>
