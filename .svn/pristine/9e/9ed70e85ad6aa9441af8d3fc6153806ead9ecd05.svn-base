...
function Calendar (cname, id, date, LangDay, LangMonth)
{
	...
	this.dayLabels = LangDay; 
	this.monthLabels = LangMonth; 	
...
{
	calHtml1 =  ("<table width='300px' height='200px' id=\"cal_" + calendar.attachedId + "\" class=\"" + calendar.calendarStyle +"\">");
...
function setFieldValue(fieldId, date) {
	document.getElementById(fieldId).value = date.getUTCDate() + "/" + (date.getUTCMonth()+1) + "/" + date.getUTCFullYear() ;
...
function setDate(date_from, date_to)  //Biolo
{
	var split_date_from = date_from.split("/");
	var year_from = split_date_from[2];
	var month_from = Number(split_date_from[1])-1;
	var day_from = Number(split_date_from[0])+1;
	var date_cal1 = new Date(year_from, month_from, day_from, 0, 0, 0);
	cal1 = new Calendar ("cal1", "tester", date_cal1 , arDay, arMonth);
	renderCalendar(cal1);
	
	var split_date_to = date_to.split("/");
	var year_to = split_date_to[2];
	var month_to = Number(split_date_to[1])-1;
	var day_to = Number(split_date_to[0])+1;
	var date_cal2 = new Date(year_to, month_to, day_to, 0, 0, 0);	
	cal2 = new Calendar ("cal2", "tester2", date_cal2 , arDay, arMonth);
	renderCalendar(cal2);
}

function verifyDate()  //Biolo
{
	var day1 = Number(document.getElementById("tester_day").value); 
	var day2 = Number(document.getElementById("tester2_day").value);
	var month1 = Number(document.getElementById("tester_month").value);
	var month2 = Number(document.getElementById("tester2_month").value);
	var year1 = Number(document.getElementById("tester_year").value);
	var year2 = Number(document.getElementById("tester2_year").value);
	
	if (year1>year2)
	{
		alert(document.getElementById("date1higher2").value);
		return false;
	}
...
function enableCal()  //Biolo
{
	document.getElementById("tester_day").disabled=false;
	document.getElementById("tester2_day").disabled=false;
	document.getElementById("tester_month").disabled=false;
	document.getElementById("tester2_month").disabled=false;
	document.getElementById("tester_year").disabled=false;
	document.getElementById("tester2_year").disabled=false;
	cal1.viewOnly= false;
	cal2.viewOnly= false;
}

function disableCal()  //Biolo
{
	document.getElementById("tester_day").disabled=true;
	document.getElementById("tester2_day").disabled=true;
	document.getElementById("tester_month").disabled=true;
	document.getElementById("tester2_month").disabled=true;
	document.getElementById("tester_year").disabled=true;
	document.getElementById("tester2_year").disabled=true;
	cal1.viewOnly= true;
	cal2.viewOnly= true;
}
