		...
		*/
		...
		if(listStatVariables != null)
		{
			for(j=0; j<listStatVariables.length; j++)
			{
				...
				vaVar = listStatVariables[j].getAttribute("valueOrig");
	...
	if(objFrmSecA != null && objStandardFrm != null)
	{
		...
		if(inputs != null)
		{
			...
			for(var i=0;i<inputs.length;i++) 
			{
				...
				{
					newHidden = null;
					if(inputs[i].type == "text" && inputs[i].disabled==false)//add condition disabled==false by Kevin, disabled input doesn't need to set
					{
