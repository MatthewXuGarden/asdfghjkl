...
{
	if(document.getElementById("deviceviewcontainer")==null) return;
	//DEV_OFFSET = (DEV_OFFSET + DEV_LIMIT);
	CommSend("servlet/ajrefresh", "POST", "cmd=mixedrfh&limit="+DEV_LIMIT+"&offset="+DEV_OFFSET+"&first="+first, "deviceviewn");
}
...
function Callback_deviceviewn()
{
	...
	var isMenuActive = xmlResponse.getElementsByTagName("isMenuActive")[0].childNodes[0].nodeValue
	...
	for (i = 0; i < listDevice.length; i++)
	{
		...
		if(deviceTable==null){ //vero --> manca riquadro --> aggiungo, sto facendo il primo giro
			...
			var y = devicecells%3;
			maintable.rows[x].cells[y].innerHTML = createDeviceTableN(listDevice[i],isMenuActive);
		} else {
...
function createDeviceTable(deviceXml){
	return createDeviceTableN(deviceXml,"true");
}
function createDeviceTableN(deviceXml,createDeviceTable){
	...
	if(createDeviceTable == "true")
		stringreturntable = stringreturntable + "onclick=\"redirect("+iddev+");"; 
	stringreturntable = stringreturntable + "\" class=\"ajaxdevtable\"><tr>" +
