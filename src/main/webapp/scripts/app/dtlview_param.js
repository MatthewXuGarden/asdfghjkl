...
function Callback_load_param(xmlResponse)
{ 
	//dichiarazioni
	...
	var	mm = "";
	var iResponse = 0;
	if( xmlResponse.firstChild.nodeName == "xml" )
		iResponse = 1;
	var	grp_id = String(xmlResponse.childNodes[iResponse].firstChild.getAttribute("id"));
	var	vars = xmlResponse.childNodes[iResponse].childNodes;
	var	variable = null; 
 	var	id = null;
	var cur_val = null;
	var type = null;
	...
	var loaded = null;
	var disab = null;
	var groups = document.getElementById("idsgroups").value.split(";");
	var input = null;
	...
	for (var i=0;i<groups.length;i++)
	{
		if (groups[i]==(grp_id))
		{
			document.getElementById("td_grp_"+groups[i]).className="groupCategorySelected";
		}
		else
		{
			document.getElementById("td_grp_"+groups[i]).className ="groupCategory";
		}
	}
	...
	var iAr = 0;
	var zebraCount =0;
	for( i = 0 , rowNumb =0 ; i < vars.length; i++)
	{
		variable = vars[i];
		if( variable.nodeName != "var" )
			continue;
	    ...
	    if(combo.length > 0)
	    {
			...
			input = createCombo(id,combo,loaded,disab,rowNumb);
			rowNumb++;
	    }
	    // FINE MODIFICA SET CON COMBO
	    else
	    {
	    	for( k = 0; k < variable.childNodes.length; k++) {
	    		 name = variable.childNodes[k].nodeName; 
	    		if( name != "#text" ) {
	    			value = variable.childNodes[k].childNodes.length ? variable.childNodes[k].childNodes[0].nodeValue : "";
	    			if(value!=null){
	    				var regN = /[\r\n]/g;
		    			value = value.replace(regN,"<br>");
		    			}
	    			eval(name + " = '" + value + "'");
	    		}
	    	}
	    ...
	    var zebraRow = "";
	    zebraRow = (zebraCount%2==0)? "Row1" : "Row2";
	    zebraCount++;
		myAr[iAr] = "<tr class='"+zebraRow +"' id='rowcur_"+id+"' onmouseover='setlongdesc("+id+","+iAr+");' onclick='setlongdesc("+id+","+iAr+");' onmouseout='document.getElementById(\"longdescdiv\").innerHTML=\"\";'>";
		myAr[iAr] += "<td id='cur_"+id+"' class='td10pt' width='12%' align='center'>" + cur_val + "</td>";
		myAr[iAr] += "<td width='18%' align='center' style='cellpadding:0px'>" + input + "</td>";
		myAr[iAr] += "<td class='td10pt' width='9%' align='center'>" + um + "</td>";
		myAr[iAr] += "<td class='td10pt' align='center' width='12%'>" + code + "</td>";
		myAr[iAr] += "<td class='td10pt' width='*' id='desccur_"+id+"'>" + description + "</td>";
		myAr[iAr] += "<td style='display: none;' id='longdescr_"+id+"' >" + longdescr + "</td></tr>";
		iAr++;
	...
	//selezione bottone configurazioni standard
	var offline = document.getElementById("offline").value;
	var hasw_v = document.getElementById("hasw_v").value;//add by Kevin
	if (grp_id<0)
	{
		linka();
		//modifiche Luca B. x disabilitare tasti se si clicca su Quick Conf:
		disableAction(3);
		disableAction(4);
		disableAction(5);
		disableAction(6);
	}
	else if(offline != 'true' && hasw_v == 'true')
	{
//		if (permission==2)
//		{
			enableAction(3);
...
//		}
	...
	//forzo riscan pagina da parte del gestore js della virtual keyboard:
	buildKeyboardInputs();
}

//html per combo box wv
//function createSetCombo(xmlResponse,type)
//{
//	var html_conf="";
//	if(xmlResponse.getElementsByTagName(type)!=null)
//		var tags=xmlResponse.getElementsByTagName(type);
//	else
//		var tags="";	
//
//	if(tags.length>0)
...
//			html_conf=html_conf+"<tr class='Row1' onmouseover='setDes()'; onmouseout='document.getElementById(\"longdescdiv\").innerHTML=\"\";'><td width='25%' align='center' class='standardTxt'>"+tags[0].childNodes[1].nodeValue+"</td>";
//		else
//			html_conf=html_conf+"<tr class='Row1' onmouseover='setDes()'; onmouseout='document.getElementById(\"longdescdiv\").innerHTML=\"\";'><td width='25%' align='center' class='standardTxt'>***</td>";
//			
//		html_conf=html_conf+"<td width='25%' align='center' class='standardTxt'><select "+vedi+" name='"+type+"_type' style='width:250px'><option value=''></option>";
//
//		for(var i=0; i<tags.length; i++)
//			html_conf=html_conf+"<option value='"+tags[i].getAttribute("id")+"' class='standardTxt'>"+tags[i].childNodes[0].nodeValue+"</option>";
//	
...
function createComboPage(xmlResponse)
{
	...
	for(var j=0;j<types.length;j++)
	{
		...
		if(tags.length>0)
		{
			...
			if(tags[0].childNodes[1].nodeValue!="")
				html_conf=html_conf+"<tr class="+ (j%2==0?"'Row1'":"Row2") +" onmouseover='setDes()'; onmouseout='document.getElementById(\"longdescdiv\").innerHTML=\"\";'><td width='25%' align='center' class='standardTxt'>"+tags[0].childNodes[1].nodeValue+"</td>";
			else
				html_conf=html_conf+"<tr class="+ (j%2==0?"'Row1'":"Row2") +" onmouseover='setDes()'; onmouseout='document.getElementById(\"longdescdiv\").innerHTML=\"\";'><td width='25%' align='center' class='standardTxt'>***</td>";
				
			html_conf=html_conf+"<td width='25%' align='center' class='standardTxt'><select "+vedi+" name='"+type+"_combo' style='width:250px'><option value=''></option>";
...
function createCombo(idvar,combo,loaded,disab,row)
{
	...
	html+="<div id='cmbcontainer_" + idvar + "' style='width: 100%; position: relative;'>";
	//x IE-7:
	html+="<select name='dtlst_" + idvar + "' id='dtlst_" + idvar + "' "+disabled+" style='width: 96.5%;'>";
	...
	{
		c = combo[j];
		c_value = c.childNodes[0].childNodes[0].nodeValue;
		c_desc = c.childNodes[1].childNodes[0].nodeValue;
...
function dtlviewSetVars()
{
	...
	{
		var noteCheck = document.getElementById("strValueNote").value;
		if(noteCheck=="yes"||noteCheck=="true"){
			document.getElementById("textareaWin").style.display="block";
			setPopupWinCenter("textareaWin");
			document.getElementById("note4setvalue").focus()
		}else{
...
function writeNote(){
	var note = document.getElementById("note4setvalue").value;
	
	if(stringtrimmer(note)==null || stringtrimmer(note)==""){
		alert(document.getElementById("noteRequired").value);
		document.getElementById("note4setvalue").focus();
	}else{
		document.getElementById("noteInfo").value=note;
		document.getElementById("cmd").value="set";
		MTstartServerComm();
		document.getElementById("frmdtlset").submit();
	}
...
function maxmincheck()
{
	...
	{
		if(inputs[i].type == "text" && inputs[i].id.match("^dtlst_") && inputs[i].value != "")
		{
...
//    retVal = window.showModalDialog( sUrl,"" ,features);
    DivWindowOpen(510,450,document.getElementById("broadcastTitle").value,sUrl,"","dtlview_param");
}
...
function chooseFileLocation(L_type){
	...
	if (document.getElementById("fileCombo"))
		document.getElementById("fileCombo").disabled=(!flag);
	if (document.getElementById("fileN"))
		document.getElementById("fileN").disabled=(!flag);
	if (document.getElementById("upload_file")) 
		document.getElementById("upload_file").disabled=flag;
	if (document.getElementById("upload_fileimg"))
		document.getElementById("upload_fileimg").style.display= (flag? "none": "block");
	if (document.getElementById("save_file")) 
		document.getElementById("save_file").disabled=flag;
	if (document.getElementById("save_fileimg")) 
		document.getElementById("save_fileimg").style.display= (flag? "none": "block");
