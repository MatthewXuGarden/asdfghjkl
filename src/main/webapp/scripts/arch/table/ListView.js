...
function LV_setPageNumber(idTable,pageNumber){
	...
	document.getElementById("page"+idTable).innerHTML = "<input type=text size='1' id='ListViewPaging_"+idTable+"' class='keyboardInput' onkeydown='checkOnlyNumber(this,event);pageSetInput_keydown(this,"+idTable+",event)' onblur='onlyNumberOnBlur(this);' value='"+this.currentPage+"'>";
...
function LV_render()
{
	...
	for (k=0; k<this.numRows; k++)
	{
		...
		else
		{
			...
			for (i=3; i<this.numCols+3; i++)
				buf += "<td align=\""+LV_decodeAlign(this.aAligCols[i-3])+"\" class=\"td\"><DIV id=\""+this.idTableNum+"divtd"+ new String(k) +""+ new String(i-3) +"\" style=\"width:"+this.aSizeDiny[i-3]+"px;height:"+rh+"\" class=\"LV_DIV\" onselectstart=\"return false;\" nowrap>"+(this.mData[k])[i]+"</DIV></td>\n";
		}
		buf += "</tr>";
		myAr[k] = buf;
		//sHtml += buf;
	...
	var sHtml = "<table id=\"TableData"+this.idTableNum+"\" class=\"table\" onMouseUp=\"document.onmousemove=null;return false\" cellspacing=\"1px\"><tbody id=\"TDBdy"+this.idTableNum+"\">"+myAr.join("")+"</tbody></table>"; 
...
function LV_getDataToPostRefresh()
{
	...
	sHtml += this.numRows+"#"+this.numCols+"#"+this.currentPage+"#refresh'></input>";
	return sHtml;	
}
...
{
	var sHtml = "<input name='pd"+this.idTableNum+"' type='hidden' value='";
	sHtml += this.numRows+"#"+this.numCols+"#"+this.currentPage+"#page'></input>";
	return sHtml;
}
...
//					str=oLswTmp.printTableDevice();
  				{  
  					CommSend("servlet/ajrefresh","POST","cmd=print_allDevices&","printTableDevice",true);
  					return;
  				}
				if(type=="alarm")
//					str=oLswTmp.printTableAlarm();
				{  
  					CommSend("servlet/ajrefresh","POST","cmd=print_allAlarms&","printTableAlarm",true);
  					return;
  				}
				if(type=="alarmcall")
...
function Callback_printTableAlarm(xml){
	...
	var str="<b style=\"font-size:18pt;\">"+t.nodeValue+"</b><br><br>"+strTime+"<br><br>\n<table  cellspacing=\"0\">";
	str+="\n<thead>";
	var ths = xml.getElementsByTagName("ths");
	var th = ths[0];
	for( var k =0;k<th.childNodes.length;k++){
		str+="\n<th class=\"thprint\">"+th.childNodes[k].childNodes[0].nodeValue+"&nbsp</th>";
	}
	str+="\n</thead>";
	str+="\n<tbody>";
	...
	for( var i =0;i<trs.length;i++){
		...
		for (var td=0;td<tr.childNodes.length ;td++){
			var cellContext;
			temp = tr.childNodes[td].childNodes[0];
			if(temp!=null ){
				cellContext = temp.nodeValue+"&nbsp";
			}else{
				cellContext = "&nbsp";
			}
...
function Callback_printTableDevice(xml){
	var sitename = document.getElementById("sitename");
	var name = "";
	if(sitename != null)
	{
	...
	var timestamp = new Date();
	var strTime = timestamp.getFullYear()+"/"+(timestamp.getMonth()+1).toString()+"/"+timestamp.getDate().toString()+" "+timestamp.toTimeString() + " - " + name;
	var str="<b style=\"font-size:18pt;\">"+t.childNodes[0].nodeValue+"</b><br><br>"+strTime+"<br><br>\n<table cellspacing=\"0\" >";
	str+="\n<thead>";
	var ths = xml.getElementsByTagName("ths");
	var th = ths[0];
	for( var k =0;k<th.childNodes.length;k++){
