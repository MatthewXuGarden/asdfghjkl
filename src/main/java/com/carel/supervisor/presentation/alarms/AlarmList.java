...
import com.carel.supervisor.presentation.devices.Device;
...
public class AlarmList
{
        ...
        int idAlarm, int idSite) throws Exception
    {
        ...
        if( page_rows != null )
        	alarmLogList.setPageRows(page_rows);
    ...
    */
        ...
        try
        {
            ...
            if (!this.visibility)
            {
                alarmLogList = page_rows != null
                	? new AlarmLogList(userSession.getLanguage(), null, BaseConfig.getPlantId(), true, page_rows.intValue(), numPage)
                	: new AlarmLogList(userSession.getLanguage(),null, BaseConfig.getPlantId(), true, numPage);
            }
            else
            {
            	alarmLogList = page_rows != null
            		? new AlarmLogList(userSession.getLanguage(), null, BaseConfig.getPlantId(), userSession.getIdSite(),true, page_rows.intValue(), idDev, numPage)
            		: new AlarmLogList(userSession.getLanguage(),null, BaseConfig.getPlantId(), userSession.getIdSite(), true, idDev, numPage);
            }
    ...
    public void loadAllFromDataBase(UserSession userSession)
    {
    	 this.visibility = userSession.localVisibility();
         ...
         try
         {
             ...
             //t(String language, String dbId, String pvcode, int idsite, boolean isActive, int[] idList)
    ...
    private HTMLElement[][] getAlarmTable(boolean group_info, UserSession us)
    {
        ...
        if (rows != 0)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                //device
                //if(!this.addLinkInDeviceName)//only show device name without link
                //{
                tableData[i][idx++] = new HTMLSimpleElement(temp.getAlarmDevice());
                //}
                //add link in device name
//                else
//                {
//                	tableData[i][idx++] = new HTMLSimpleElement("<a href='javascript:void(0)'  onclick=top.frames['manager'].loadTrx('nop&folder=dtlview&bo=BDtlView&type=click&iddev="+temp.getAlarmIDDevice()+"&desc=ncode01')>"+temp.getAlarmDevice()+"</a>");
//                }
//                
                //link
                if(isLink==true){
	                if(temp.getAlarmIDDevice() != -1 && us.isMenuActive("dtlview")){
	                	...
	                	tableData[i][idx++] = new HTMLSimpleElement("<img src='images/system/right.png' class='imglink' title='" + tooltip + "' onclick=event.stopPropagation();top.frames['manager'].loadTrx('dtlview/FramesetTab.jsp&folder=dtlview&bo=BDtlView&type=click&iddev="+temp.getAlarmIDDevice()+"&desc=ncode01') />");
	                }else{
	                	tableData[i][idx++] = new HTMLSimpleElement("");
	                }
                }
	...
	//                			if (AlarmMngTable.mandatoryNote(new Integer(temp.getPriority()))){
...
//            alarmTable.setAlignType(colspan, 1);//7
    ...
    public Object getAllAlarms4Print(String lan, LangService multiLanguage, UserSession us) {
    	StringBuffer sb = new StringBuffer();
    	sb.append("<mytitle>"+ "<![CDATA[" + this.title  + "]]>" + "</mytitle>");
    	String pv_type = BaseConfig.getProductInfo("type");
        boolean group_info = (pv_type.equalsIgnoreCase(BaseConfig.ADVANCED_TYPE))
            ? true : false;
	    String header[] = getHeaderAlarmTable(lan, group_info); 
	    sb.append("<ths>");
    	...
    	for(int i=0;i<alarms.size();i++ ){
    		sb.append("<tr>");
    		Alarm temp = ((Alarm) alarms.get(i));
    		sb.append("<td >"+"<![CDATA[" +"<div style='visibility:hidden;display:none'>" +
                    getLongDate(temp.getDate()) + "</div>" +
                    temp.formatDate(temp.getDate())+"]]>" +"</td>");
    		sb.append("<td>"+"<![CDATA[" +temp.getAlarmDevice()+"]]>" +"</td>");
    		
    		if(isLink==true){
                if(temp.getAlarmIDDevice() != -1 && us.isMenuActive("dtlview")){
                	String tooltip = multiLanguage.getString("alarmTable", "dtltooltip");
                	sb.append("<td>"+"<![CDATA[" +"<img src='images/system/right.png' />"+"]]>" +"</td>");
                }else{
    		...
    		String tmp = temp.getAlarmVariable();
			NoteLogList listaNote = new NoteLogList();
            try
            {
				listaNote.retrieve(temp.getSite(), "hsalarm", temp.getId());
            ...
            {
            case (1):
                priority = multiLanguage.getString("alrview", "alarmstate1");

                break;

            case (2):
                priority = multiLanguage.getString("alrview", "alarmstate2");

                break;

            case (3):
                priority = multiLanguage.getString("alrview", "alarmstate3");

                break;

            case (4):
                priority = multiLanguage.getString("alrview", "alarmstate4");

                break;
            }
            sb.append("<td>"+"<![CDATA[" +priority+"]]>" +"</td>");
            ...
            if(this.showAckCancel)
            {
                ...
                if (temp.getAckuser() != null)
                {
                    sb.append("<td>"+"<![CDATA[" + "<img src='images/ok.gif'/>"+"]]>" +"</td>");
                    this.num_ack++;
                }
                else
                {
...
//                		if (listaNote!=null && listaNote.size()==0){
//                			if (AlarmMngTable.mandatoryNote(new Integer(temp.getPriority()))){
        				ack="<img title='"+multiLanguage.getString("alrmng", "noterequired")+"' src='images/genericdanger.gif'/>";
//                			}
//                		}
                	}
                	sb.append("<td>"+"<![CDATA[" + ack+"]]>" +"</td>");
                }
    ...
    private HTMLElement[][] getAlarmFindTable(boolean group_info, UserSession us)
    {
        ...
        if (rows != 0)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                {
                	String tooltip = lan.getString("alarmTable", "dtltooltip");
                	tmp = "<img src='images/system/right.png' class='imglink' title='" + tooltip + "' onclick=event.stopPropagation();top.frames['manager'].loadTrx('dtlview/FramesetTab.jsp&folder=dtlview&bo=BDtlView&type=click&iddev="+temp.getAlarmIDDevice()+"&desc=ncode01') />";
                }
                else
                {
                	tmp = "";
