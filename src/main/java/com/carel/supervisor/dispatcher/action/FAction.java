...
public class FAction extends DispatcherAction
{
    public static final String SPLIT = ";";

    public FAction(Integer key, Integer id, Integer pri, Integer sta, String rec, Timestamp itime,
    ...
    private void generateRtfDocs(String pathDir) throws Exception
    {
    	...
    	try
        {
	    	...
	    	for (int i = 0; i < receivers.length; i++)
	        {
				...
				p.setFont(helvetica11Bold);
				p.add("To No. / A Num. / Zu Nr. / � No. / A Nu.:");
				p.setFont(helvetica11);
				...
				if (this.isAlarm())
                {
                    document = buildBodyAlarm(infoSender[2], document);
                }
                else
                {
                    document = buildBodyAlive(infoSender[2], document);
                }
