...
public class WriteDataTest extends Poller
{
    ...
    public void executeLoop() throws DataBaseException, SQLException
    {
        try
        {
        ...
        try
        {
            ...
            do
            {
                ...
                {
                    Connection connection = DatabaseMgr.getInstance()
                                                       .getConnection(null);
                    PreparedStatement preparedStatement = connection.prepareStatement(
                            "SELECT keymax,keyactual,isturn from buffer");
                    ResultSet resultSet = preparedStatement.executeQuery();
                    resultSet.next();

                    int keyMax = resultSet.getShort("keymax");
                    int keyActual = resultSet.getShort("keyactual");
                    boolean isTurn = resultSet.getBoolean("isturn");
    ...
    private static void request() throws Exception
    {
        ...
        try
        {
            ...
            System.out.print("Settare numero valori ");
            str = myInput.readLine();
            NUMERO_DI_GIRI = Integer.parseInt(str);

            System.out.print("Settare il key max ");
            str = myInput.readLine();

            int keyMax = Integer.parseInt(str);
            Connection connection = DatabaseMgr.getInstance().getConnection(null);
            PreparedStatement preparedStatement = connection.prepareStatement(
                    "update buffer set keymax=" + keyMax);
            preparedStatement.execute();
            preparedStatement.close();
            connection.commit();
            connection.close();

            if ((NUMERO_DI_GIRI / 64) < keyMax)
            {
                System.out.println("Non farai il giro INSERT");
            }
            else
            {
                System.out.println("Giro UPDATE");
            }
        } //try 
        catch (IOException e)
        {
