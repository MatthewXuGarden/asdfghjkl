...
public class SwitchCallBack extends DefaultCallBack 
{
	...
	public void onStart(SetContext setContext)
	{
		...
		{
			EventMgr.getInstance().info(new Integer(1), setContext.getUser(), "switch",
	...
	public void onStop(SetContext setContext)
	{
		if (setContext.isLoggable())
		{
			EventMgr.getInstance().warning(new Integer(1), setContext.getUser(), "switch",
	...
	{int idEvent=-1;
		if (errors)
		{
			if (setContext.isLoggable())
			{
				idEvent=EventMgr.getInstance().error(new Integer(1), setContext.getUser(), "switch",
		...
		else
		{
			if (setContext.isLoggable())
			{
				idEvent=EventMgr.getInstance().info(new Integer(1), setContext.getUser(), "switch",
						"W051", new Object[] { ""+setContext.getID()});
			}
		}
		return idEvent;
